...
export default function TabTwoScreen(){
    const {user, isAuthenticated, login, register, logout, isLoadingSession, updatePerfil} = useAuth() as AuthCtx;
    ...
    const [successMessage, setSuccessMensaje] =useState('');
    ...
    const handleGuardarPerfil = async () => {
        ...
        try{
            // solo envia los campos que tiene valor los vacios se emite
            ...
            setEditMode(false); // vuelve al modo lectura tras actualizar
            // limpia los campos editables para que el usuario vea los cambios reflejados
            setEditNombre('');
            setEditEmail('');
            setEditPassword('');
        } catch (error: unknown){
            setPerfilError((error as {message?:string})?.message || 'no fue posible actualiza perfil')
        }finally{
