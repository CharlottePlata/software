...
export default function HomeScreen(){
    ...
    /**
     * Funcion handleLoadData 
     * agrega el producto al carrito y muestra una alerta de confirmacion 
     * o error
     */
     ...
     * carga los productos y categorias desde el backend 
     * en paralelo con promise.all
     * isRefresh´: true cuando el pull tu refresh 
     */
    ...
    const loadCatalogo = async ({ isRefresh = false} = {}) => {
        ...
        try {
            /**
             * llama todos los endpointd ak mismo tiempo para mayor velocidad
             *  getproductos con liite de 200 trae todos los productos de una vez
             */
     ...
     * siempre vuelve a la pagina 1 para no quedar rn una pagia vacia
     */
     ...
     * usememo recalcula solo cuando cambia las dependeciuas indicadas
     * evita  filtrar/calcualr en cada renderizado
     */

    // Productos filtrados subcojunto de productos que coinciden con la busqueda y categoria
    const productosFiltrados = useMemo(() => {
        ...
        return productos.filter((p: any) => {
            ...
            // categoriaId o categoria.id son los dos campos posibles seugn el endpoint
            const coincideCategoria = categoriaActiva === 'all' ||
            // pasa todo
            String(p.categoriaId || p.categoria?.id) === (categoriaActiva); 
            return coincideTexto && coincideCategoria;
        });
    }, [busqueda, categoriaActiva, productos]);
    // recalcula cuando cambia la busqueda caegoria o dato

    /**
     * hasProductos: true si hay al menos un productos filtrado evitar motrar la vista vacida
     */
    const hasProductos = useMemo(()=>
    ...
    // productosnvisibles sunconjuntos de productos de la pagina actual eje pagina 1 
    // 1-15 pagina 2 16-30 etc..

    const productosVisibles = useMemo(() => productosFiltrados.slice((paginaActual - 1)
