/**
 *  Pnatalla principal del panel del administrador y auxiliar
 * solo accedible para roles definidos 
 * muestra tarjetas de estadisticas en tiempo real
 * pedidos / ventas totals
 * los auxiliares ven todo excepto la tarjeta de usuarios
 * isAdmin =  false
 * incluye accesos rapiso y las secciones mas usadas 
 * muestra informacion del sistema (estado de la api y 
 * rol de usuario)
 */
...
/**
 * Panel de navegacion
 * el tipo de router de expo no expone si el push con tipiado correcto
 * las versiones actuales del os y Android no permiten uso del push 
 * 
 */
...
// tipo statCard
// describe la foma de cada tarjeta de estadisticas de grid
...
export default function AdminDashboardScreen(){
    ...
    const isAdmin = user?.rol === 'administrador'; // true solo para adiministradores
    ...
    useEffect(() => {
        const load = async () =>{
            setLoading(true);
            try {
                // se hacen 4 peticiones en paralelo con promise.all para reproducir el tiempo de carga
                // cada uno trae datos de una seccion diferente al panel
                const [cats, subs, prods, orders] = await Promise.all([
                    apiClient.get('/admin/categorias'), // numero total de categorias
                    apiClient.get('/admin/subcategorias'), // numero total de subcategorias
                    apiClient.get('/admin/productos? limite=1'), // numero total de productos
                    apiClient.get('/admin/pedidos/estadisticas'), // resumen de pedidos con totales y pendientes
                ])

                // los stats de usuario solo se piden si el ussuario es admin
                //  nlos auxiliares no tiene acceso a ese endpoint
                const userStats= isAdmin? await apiClient.get('/admin/usuarios/stats') : null;

                // extraelos datos de cada respuesta con optional chaining y fallback
                const catsData = cats.data?.data?.categorias || {};
                const subsData = subs.data?.data?.subcategorias || {};
                const ordStats = orders.data?.data?.stats || {};

                // actualiza el estado con todo los contadores de calculos
                setStats({
                ...
                // si alguna  peticion falla simplente se ignora el error
