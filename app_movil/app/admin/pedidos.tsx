...
export default function AdminPedidoScreen(){
    ...
    // texto actual de la busqueda
    const [pagina, setPagina] = useState(1);
    // pagina actual es la uno
     ...
     * page numero de la pgania a cargar 
    ...
    const fetchPedidos = async (page = 1, search= '') => {
        ...
        setErrorMessage('');
        try{
            // construye el quey del string dinamicamente
            const params: string [] = [];
            if(search.trim ()) params.push(`buscar=${encodeURIComponent(search.trim())}`); // esto codifica los caracteres especiales en la buqueda
            params.push(`pagina =${page}`); 
            // numero de la pagina
            params.push(`limite = 10 `); 
            // 10 pedidos por pagina
            ...
            // la que extrae los pedidos de la array
            ...
            setPedidos(pedidosData); // actualiza el estado de los pedidos obtenidos
            setPagina
            (page);
            ...
            setTotalPaginas(res.data?.data?.paginacion?.totalPaginas || 1)
                ...
                setErrorMessage((error as {message?:string})?.message || 'no se puede cargar el pedido'); 
            }finally{
         ...
         * Math.max y math.min evita ir mas alla de los limites de pagina disponibles
         */

        const handlePagina = (next: number) => {
            const nueva = Math.max(1, Math.min
            (totalPaginas, pagina + next));
